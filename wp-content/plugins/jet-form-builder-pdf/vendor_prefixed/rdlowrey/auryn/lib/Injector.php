...
class Injector
{
    ...
    public function alias($original, $alias)
    {
        ...
        if (isset($this->shares[$originalNormalized])) {
            throw new ConfigException(\sprintf(self::M_SHARED_CANNOT_ALIAS, $this->normalizeName(\get_class($this->shares[$originalNormalized])), $alias), self::E_SHARED_CANNOT_ALIAS);
        }
    ...
    public function share($nameOrInstance)
    {
        if (\is_string($nameOrInstance)) {
            ...
            throw new ConfigException(\sprintf(self::M_SHARE_ARGUMENT, __CLASS__, \gettype($nameOrInstance)), self::E_SHARE_ARGUMENT);
        }
        return $this;
    }
    ...
    private function shareInstance($obj)
    {
        ...
        if (isset($this->aliases[$normalizedName])) {
            ...
            throw new ConfigException(\sprintf(self::M_ALIASED_CANNOT_SHARE, $normalizedName, $this->aliases[$normalizedName]), self::E_ALIASED_CANNOT_SHARE);
        }
    ...
    public function prepare($name, $callableOrMethodStr)
    {
        if ($this->isExecutable($callableOrMethodStr) === \false) {
            throw InjectionException::fromInvalidCallable($this->inProgressMakes, $callableOrMethodStr);
        }
        list(, $normalizedName) = $this->resolveAlias($name);
    ...
    public function delegate($name, $callableOrMethodStr)
    {
        if ($this->isExecutable($callableOrMethodStr) === \false) {
            ...
            throw new ConfigException(\sprintf(self::M_DELEGATE_ARGUMENT, __CLASS__, $errorDetail), self::E_DELEGATE_ARGUMENT);
        }
        $normalizedName = $this->normalizeName($name);
    ...
    public function make($name, array $args = array())
    {
        ...
        if (isset($this->inProgressMakes[$normalizedClass])) {
            throw new InjectionException($this->inProgressMakes, \sprintf(self::M_CYCLIC_DEPENDENCY, $className), self::E_CYCLIC_DEPENDENCY);
        }
    ...
    private function provisionInstance($className, $normalizedClass, array $definition)
    {
        try {
            ...
            if (!$ctor) {
                ...
                throw new InjectionException($this->inProgressMakes, \sprintf(self::M_NON_PUBLIC_CONSTRUCTOR, $className), self::E_NON_PUBLIC_CONSTRUCTOR);
            ...
            throw new InjectionException($this->inProgressMakes, \sprintf(self::M_MAKE_FAILURE, $className, $e->getMessage()), self::E_MAKE_FAILURE, $e);
        }
    }
    private function instantiateWithoutCtorParams($className)
    {
        ...
        if (!$reflClass->isInstantiable()) {
            ...
            throw new InjectionException($this->inProgressMakes, \sprintf(self::M_NEEDS_DEFINITION, $type, $className), self::E_NEEDS_DEFINITION);
        }
    ...
    private function buildArgFromParamDefineArr($definition)
    {
        if (!\is_array($definition)) {
            throw new InjectionException($this->inProgressMakes);
        }
    ...
    private function buildArgFromReflParam(\ReflectionParameter $reflParam, $className = null)
    {
        if (\array_key_exists($reflParam->name, $this->paramDefinitions)) {
            ...
            throw new InjectionException($this->inProgressMakes, \sprintf(self::M_UNDEFINED_PARAM, $reflParam->name, $reflParam->getPosition(), $funcWord, $classDeclare), self::E_UNDEFINED_PARAM);
        }
        return $arg;
    }
    private function prepareInstance($obj, $normalizedClass)
    {
        ...
        if ($interfaces === \false) {
            throw new InjectionException($this->inProgressMakes, \sprintf(self::M_MAKING_FAILED, $normalizedClass, \gettype($obj)), self::E_MAKING_FAILED);
        }
        if (empty($interfaces)) {
            return $obj;
        }
    ...
    public function buildExecutable($callableOrMethodStr)
    {
        try {
            ...
            throw InjectionException::fromInvalidCallable($this->inProgressMakes, $callableOrMethodStr, $e);
        }
    ...
    private function buildExecutableStructFromString($stringExecutable)
    {
        if (\function_exists($stringExecutable)) {
            ...
            throw InjectionException::fromInvalidCallable($this->inProgressMakes, $stringExecutable);
        }
        return $executableStruct;
    }
    ...
    private function buildExecutableStructFromArray($arrayExecutable)
    {
        ...
        if (\is_object($classOrObj) && \method_exists($classOrObj, $method)) {
            ...
            throw InjectionException::fromInvalidCallable($this->inProgressMakes, $arrayExecutable);
        }
        return $executableStruct;
    }
}
