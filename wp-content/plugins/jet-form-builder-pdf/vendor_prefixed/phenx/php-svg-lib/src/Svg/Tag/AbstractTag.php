...
use JFB_PDF\Vendor\Svg\CssLength;
...
abstract class AbstractTag
{
    ...
    protected function applyTransform($attributes)
    {
        if (isset($attributes["transform"])) {
            ...
            $matches = array();
            \preg_match_all('/(matrix|translate|scale|rotate|skew|skewX|skewY)\\((.*?)\\)/is', $transform, $matches, \PREG_SET_ORDER);
            ...
            foreach ($transformations as $t) {
                switch ($t[0]) {
                    ...
                    case "rotate":
                        ...
                        break;
                    case "skewX":
                        $tan_x = \tan(\deg2rad($t[1]));
                        $surface->transform(1, 0, $tan_x, 1, 0, 0);
                        break;
                    case "skewY":
                        $tan_y = \tan(\deg2rad($t[1]));
