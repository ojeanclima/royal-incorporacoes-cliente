 ...
 */
namespace JFB_PDF\Vendor\Svg;

use JFB_PDF\Vendor\Svg\Tag\AbstractTag;
class Style
{
    const TYPE_COLOR = 1;
    const TYPE_LENGTH = 2;
    ...
    {
        return array('color' => array('color', self::TYPE_COLOR), 'opacity' => array('opacity', self::TYPE_NUMBER), 'display' => array('display', self::TYPE_NAME), 'fill' => array('fill', self::TYPE_COLOR), 'fill-opacity' => array('fillOpacity', self::TYPE_NUMBER), 'fill-rule' => array('fillRule', self::TYPE_NAME), 'stroke' => array('stroke', self::TYPE_COLOR), 'stroke-dasharray' => array('strokeDasharray', self::TYPE_NAME), 'stroke-dashoffset' => array('strokeDashoffset', self::TYPE_NUMBER), 'stroke-linecap' => array('strokeLinecap', self::TYPE_NAME), 'stroke-linejoin' => array('strokeLinejoin', self::TYPE_NAME), 'stroke-miterlimit' => array('strokeMiterlimit', self::TYPE_NUMBER), 'stroke-opacity' => array('strokeOpacity', self::TYPE_NUMBER), 'stroke-width' => array('strokeWidth', self::TYPE_NUMBER), 'font-family' => array('fontFamily', self::TYPE_NAME), 'font-size' => array('fontSize', self::TYPE_NUMBER), 'font-weight' => array('fontWeight', self::TYPE_NAME), 'font-style' => array('fontStyle', self::TYPE_NAME), 'text-anchor' => array('textAnchor', self::TYPE_NAME));
    }
    /**
     * @param $attributes
     *
    ...
    {
        $class = isset($attributes["class"]) ? \preg_split('/\\s+/', \trim($attributes["class"])) : null;
            ...
            if (isset($styles[$from])) {
                ...
                switch ($type) {
                    case self::TYPE_COLOR:
                        ...
                        if (\is_array($value) && $value[3] !== 1.0 && \array_key_exists("{$from}-opacity", $style_map) === \true) {
                ...
                if ($from === "font-family") {
                    ...
                    if ($scheme === "phar" || \strtolower(\substr($value, 0, 7)) === "phar://" || $this->_document !== null && $this->_document->allowExternalReferences === \false && $scheme !== "data") {
                        continue;
                    }
                }
                if ($value !== null) {
    ...
    static function parseColor($color)
    {
        ...
        if (\strpos($color, "hsl") !== \false) {
            ...
            $a = $a * 255;
            return array($r * 255.0, $g * 255.0, $b * 255.0, $a);
        }
        // Gradient
    ...
    static function getQuad($color, $percent = \false)
    {
        ...
        $quad = \preg_split("/\\s*[,\\/]\\s*/", \trim(\substr($color, $i + 1, $j - $i - 1)));
        if (!isset($quad[3])) {
            $quad[3] = "1";
        }
