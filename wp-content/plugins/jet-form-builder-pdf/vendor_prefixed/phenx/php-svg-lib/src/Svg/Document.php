...
use JFB_PDF\Vendor\Svg\Surface\SurfaceInterface;
use JFB_PDF\Vendor\Svg\Tag\AbstractTag;
use JFB_PDF\Vendor\Svg\Tag\Anchor;
use JFB_PDF\Vendor\Svg\Tag\Circle;
use JFB_PDF\Vendor\Svg\Tag\Ellipse;
use JFB_PDF\Vendor\Svg\Tag\Group;
use JFB_PDF\Vendor\Svg\Tag\ClipPath;
use JFB_PDF\Vendor\Svg\Tag\Image;
use JFB_PDF\Vendor\Svg\Tag\Line;
use JFB_PDF\Vendor\Svg\Tag\LinearGradient;
use JFB_PDF\Vendor\Svg\Tag\Path;
use JFB_PDF\Vendor\Svg\Tag\Polygon;
use JFB_PDF\Vendor\Svg\Tag\Polyline;
use JFB_PDF\Vendor\Svg\Tag\Rect;
use JFB_PDF\Vendor\Svg\Tag\Stop;
use JFB_PDF\Vendor\Svg\Tag\Symbol;
use JFB_PDF\Vendor\Svg\Tag\Text;
use JFB_PDF\Vendor\Svg\Tag\StyleTag;
use JFB_PDF\Vendor\Svg\Tag\UseTag;
class Document extends AbstractTag
{
    ...
    public $allowExternalReferences = \true;
    ...
    public function exitDefs()
    {
        ...
        $this->inDefs = $this->_defs_depth > 0 ? \true : \false;
        return $this->_defs_depth;
    }
    /**
    ...
    public function getDimensions()
    {
        ...
        $parser = \xml_parser_create("utf-8");
        \xml_parser_set_option($parser, \XML_OPTION_CASE_FOLDING, \false);
        \xml_set_element_handler($parser, function ($parser, $name, $attributes) use(&$rootAttributes) {
    ...
    public function handleSizeAttributes($attributes)
    {
        if ($this->width === null) {
            ...
            if (isset($attributes['viewbox'])) {
                $viewBox = \preg_split('/[\\s,]+/is', \trim($attributes['viewbox']));
                if (\count($viewBox) == 4) {
                    $this->x = $viewBox[0];
                    $this->y = $viewBox[1];
                    if (!$this->width) {
