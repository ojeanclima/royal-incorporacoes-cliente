 ...
 */
namespace JFB_PDF\Vendor\FontLib\Table\Type;
...
class cmap extends Table
{
    ...
    private static $subtable_v2_format = array("length" => self::uint16, "language" => self::uint16);
    private static $subtable_v2_format_subheader = array("firstCode" => self::uint16, "entryCount" => self::uint16, "idDelta" => self::int16, "idRangeOffset" => self::uint16);
        ...
        foreach ($data["subtables"] as $i => &$subtable) {
            ...
            switch ($subtable["format"]) {
                ...
                case 2:
                    ...
                    $subtable += array("subHeaderKeys" => $subHeaderKeys, "subHeaders" => $subHeaders, "glyphIdArray" => $glyphIdArray, "glyphIndexArray" => $glyphIndexArray);
                    break;
                case 4:
    ...
    function _encode()
    {
        ...
        // size of self::$subtable_header_format
        $subtable_headers_offset = $font->pos();
        $length += $font->write(\str_repeat("\x00", $subtable_headers_size), $subtable_headers_size);
