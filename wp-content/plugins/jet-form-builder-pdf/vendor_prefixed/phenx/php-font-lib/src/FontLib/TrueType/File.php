 ...
 */
namespace JFB_PDF\Vendor\FontLib\TrueType;

use JFB_PDF\Vendor\FontLib\AdobeFontMetrics;
use JFB_PDF\Vendor\FontLib\Font;
use JFB_PDF\Vendor\FontLib\BinaryStream;
use JFB_PDF\Vendor\FontLib\Table\Table;
use JFB_PDF\Vendor\FontLib\Table\DirectoryEntry;
use JFB_PDF\Vendor\FontLib\Table\Type\glyf;
use JFB_PDF\Vendor\FontLib\Table\Type\name;
use JFB_PDF\Vendor\FontLib\Table\Type\nameRecord;
/**
 * TrueType font file.
 *
...
class File extends BinaryStream
{
    ...
    function getUnicodeCharMap()
    {
        ...
        $system_encodings = \array_change_key_case(\array_fill_keys($system_encodings, \true), \CASE_UPPER);
        foreach ($this->getData("cmap", "subtables") as $_subtable) {
            $encoding = null;
    ...
    function parseTableEntries()
    {
        ...
        $class = "JFB_PDF\\Vendor\\FontLib\\{$type}\\TableDirectoryEntry";
    ...
    protected function readTable($tag)
    {
        ...
        if (!self::$raw) {
            ...
            $class = "JFB_PDF\\Vendor\\FontLib\\Table\\Type\\{$name_canon}";
            if (!isset($this->directory[$tag]) || !@\class_exists($class)) {
                return;
            }
        } else {
            $class = "JFB_PDF\\Vendor\\FontLib\\Table\\Table";
        }
        /** @var Table $table */
    ...
    function reduce()
    {
        $names_to_keep = array(name::NAME_COPYRIGHT, name::NAME_NAME, name::NAME_SUBFAMILY, name::NAME_SUBFAMILY_ID, name::NAME_FULL_NAME, name::NAME_VERSION, name::NAME_POSTSCRIPT_NAME);
