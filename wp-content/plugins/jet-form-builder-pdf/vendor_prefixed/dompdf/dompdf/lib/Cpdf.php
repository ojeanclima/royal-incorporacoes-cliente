 ...
 * @package Cpdf
 */
namespace JFB_PDF\Vendor\Dompdf;

use JFB_PDF\Vendor\FontLib\Exception\FontNotFoundException;
use JFB_PDF\Vendor\FontLib\Font;
use JFB_PDF\Vendor\FontLib\BinaryStream;
class Cpdf
{
     ...
     */
    public $options = ['compression' => \true];
    /**
    ...
    protected function o_font($id, $action, $options = '')
    {
        ...
        switch ($action) {
            case 'new':
                $this->objects[$id] = ['t' => 'font', 'info' => ['name' => $options['name'], 'fontFileName' => $options['fontFileName'], 'SubType' => 'Type1', 'isSubsetting' => $options['isSubsetting']]];
    ...
    private function processFont(int $fontObjId, array $object_info)
    {
        ...
        if (!$fileSuffix) {
            ...
            if (!$font['isSubsetting']) {
                ...
                $font_obj->reduce();
                // Write new font
                $tmp_name = @\tempnam($this->tmp, "cpdf_subset_");
            ...
            $fdopt = ['Flags' => $flags, 'FontName' => $adobeFontName, 'StemV' => $stemV];
            foreach ($list as $k => $v) {
                if (isset($font[$v])) {
                    $fdopt[$k] = $font[$v];
                }
            ...
            if ($isPfbFont) {
                ...
                $this->o_contents($this->numObj, 'add', ['Length1' => $l1, 'Length2' => $l2, 'Length3' => $l3]);
            } elseif ($isTtfFont) {
                $l1 = \mb_strlen($data, '8bit');
                $this->o_contents($this->numObj, 'add', ['Length1' => $l1]);
            }
            // tell the font object about all this new stuff
            $options = ['BaseFont' => $adobeFontName, 'MissingWidth' => $missing_width, 'Widths' => $widthid, 'FirstChar' => $firstChar, 'LastChar' => $lastChar, 'FontDescriptor' => $fontDescriptorId];
            if ($isTtfFont) {
...
EOT;
    ...
    protected function o_cidSystemInfo($id, $action)
    {
        switch ($action) {
            case 'new':
                $this->objects[$id] = ['t' => 'cidSystemInfo'];
                break;
            case 'add':
                break;
            case 'out':
    ...
    protected function o_info($id, $action, $options = '')
    {
        switch ($action) {
            case 'new':
                $this->infoObject = $id;
                $date = 'D:' . @\date('Ymd');
                $this->objects[$id] = ['t' => 'info', 'info' => ['Producer' => 'CPDF (dompdf)', 'CreationDate' => $date]];
                break;
            case 'Title':
            case 'Author':
            case 'Subject':
    ...
    protected function o_page($id, $action, $options = '')
    {
        ...
        switch ($action) {
            case 'new':
                $this->numPages++;
                $this->objects[$id] = ['t' => 'page', 'info' => ['parent' => $this->currentNode, 'pageNum' => $this->numPages, 'mediaBox' => $this->objects[$this->currentNode]['info']['mediaBox']]];
                if (\is_array($options)) {
    ...
    protected function o_embedjs($id, $action)
    {
        switch ($action) {
            case 'new':
                $this->objects[$id] = ['t' => 'embedjs', 'info' => ['Names' => '[(EmbeddedJS) ' . ($id + 1) . ' 0 R]']];
                break;
            case 'out':
                $o =& $this->objects[$id];
                $res = "\n{$id} 0 obj\n<< ";
    ...
    protected function o_javascript($id, $action, $code = '')
    {
        switch ($action) {
            case 'new':
                $this->objects[$id] = ['t' => 'javascript', 'info' => ['S' => '/JavaScript', 'JS' => '(' . $this->filterText($code, \true, \false) . ')']];
                break;
            case 'out':
                $o =& $this->objects[$id];
                $res = "\n{$id} 0 obj\n<< ";
                                ...
                                if (isset($options['transparency'])) {
                                    ...
                                    switch ($transparency['type']) {
                                        case 'indexed':
                                            ...
                                            break;
                                        case 'color-key':
                                            $tmp = ' [ ' . $transparency['r'] . ' ' . $transparency['r'] . $transparency['g'] . ' ' . $transparency['g'] . $transparency['b'] . ' ' . $transparency['b'] . ' ] ';
                                            $info['Mask'] = $tmp;
                                            break;
                                    }
                                }
    ...
    {
        static $valid_params = ["LW", "LC", "LC", "LJ", "ML", "D", "RI", "OP", "op", "OPM", "Font", "BG", "BG2", "UCR", "TR", "TR2", "HT", "FL", "SM", "SA", "BM", "SMask", "CA", "ca", "AIS", "TK"];
        switch ($action) {
            case "new":
    ...
    protected function o_xobject($id, $action, $options = '')
    {
        switch ($action) {
            ...
            case 'procset':
                ...
                break;
            case 'font':
                $this->objects[$id]['fonts'][$options['fontNum']] = ['objNum' => $options['objNum'], 'fontNum' => $options['fontNum']];
                break;
            case 'xObject':
    ...
    protected function o_sig($id, $action, $options = '')
    {
        ...
        switch ($action) {
            ...
            case 'byterange':
                ...
                if (\file_put_contents($tmpInput, \substr($content, $rangeStartPos + 2 + $sign_maxlen), \FILE_APPEND) === \false) {
                    throw new \Exception("Unable to write temporary file for signing.");
                }
                if (\openssl_pkcs7_sign($tmpInput, $tmpOutput, $o['info']['SignCert'], array($o['info']['PrivKey'], $o['info']['Password']), array(), \PKCS7_BINARY | \PKCS7_DETACHED) === \false) {
            ...
            case "out":
                ...
                foreach ($o['info'] as $k => $v) {
                    switch ($k) {
                        ...
                        case 'ContactInfo':
                            if ($v !== null && $v !== '') {
                                $res .= "/{$k} (" . ($encrypted ? $this->filterText($this->ARC4($v), \false, \false) : $v) . ") \n";
                            }
                            break;
                    }
                }
    ...
    protected function o_encryption($id, $action, $options = '')
    {
        switch ($action) {
            ...
            case 'keys':
                ...
                $pad = \chr(0x28) . \chr(0xbf) . \chr(0x4e) . \chr(0x5e) . \chr(0x4e) . \chr(0x75) . \chr(0x8a) . \chr(0x41) . \chr(0x64) . \chr(0x0) . \chr(0x4e) . \chr(0x56) . \chr(0xff) . \chr(0xfa) . \chr(0x1) . \chr(0x8) . \chr(0x2e) . \chr(0x2e) . \chr(0x0) . \chr(0xb6) . \chr(0xd0) . \chr(0x68) . \chr(0x3e) . \chr(0x80) . \chr(0x2f) . \chr(0xc) . \chr(0xa9) . \chr(0xfe) . \chr(0x64) . \chr(0x53) . \chr(0x69) . \chr(0x7a);
                $info = $this->objects[$id]['info'];
    ...
    private function openFont($font)
    {
        ...
        if (\file_exists($fontcache . '/' . $cache_name)) {
            ...
            $cached_font_info = \json_decode(\file_get_contents($fontcache . '/' . $cache_name), \true);
                ...
                if ($pos) {
                    ...
                    switch ($key) {
                        ...
                        case 'FontHeightOffset':
                            ...
                            $data[$key] = \trim(\substr($row, $pos));
                            break;
                        case 'FontBBox':
                            $data[$key] = \explode(' ', \trim(\substr($row, $pos)));
                            break;
     ...
     * @throws FontNotFoundException
     */
    function selectFont($fontName, $encoding = '', $set = \true, $isSubsetting = \true)
    {
    ...
    {
        static $blend_modes = ["Normal", "Multiply", "Screen", "Overlay", "Darken", "Lighten", "ColorDogde", "ColorBurn", "HardLight", "SoftLight", "Difference", "Exclusion"];
        ...
        if ($mode === (\key_exists('mode', $this->currentLineTransparency) ? $this->currentLineTransparency['mode'] : '') && $opacity === (\key_exists('opacity', $this->currentLineTransparency) ? $this->currentLineTransparency["opacity"] : '')) {
            return;
        }
        $this->currentLineTransparency["mode"] = $mode;
        $this->currentLineTransparency["opacity"] = $opacity;
        $options = ["BM" => "/{$mode}", "CA" => (float) $opacity];
    ...
    function setFillTransparency($mode, $opacity)
    {
        ...
        if ($mode === (\key_exists('mode', $this->currentFillTransparency) ? $this->currentFillTransparency['mode'] : '') && $opacity === (\key_exists('opacity', $this->currentFillTransparency) ? $this->currentFillTransparency["opacity"] : '')) {
            return;
        }
     ...
     * @param bool  $stroke
     */
    function line($x1, $y1, $x2, $y2, $stroke = \true)
    {
    ...
    {
        $this->ellipse($x0, $y0, $r1, $r2, $angle, $nSeg, $astart, $afinish, \true, \true);
    }
    /**
     * @param float $x
     * @param float $y
     ...
     */
    function ellipse($x0, $y0, $r1, $r2 = 0, $angle = 0, $nSeg = 8, $astart = 0, $afinish = 360, $close = \true, $fill = \false, $stroke = \true, $incomplete = \false)
    {
        if ($r1 == 0) {
            return;
        }
    ...
    {
        $sigId = ++$this->numObj;
        $this->o_sig($sigId, 'new', ['SignCert' => $signcert, 'PrivKey' => $privkey, 'Password' => $password, 'Name' => $name, 'Location' => $location, 'Reason' => $reason, 'ContactInfo' => $contactinfo]);
        return $sigId;
    }
    /**
     * add field to form
    ...
    public function addFormField($type, $name, $x0, $y0, $x1, $y1, $ff = 0, $size = 10.0, $color = [0, 0, 0])
    {
        ...
        $font = \array_filter($this->objects[$this->currentNode]['info']['fonts'], function ($item) use($currentFontNum) {
            return $item['fontNum'] == $currentFontNum;
        });
        $this->o_acroform($this->acroFormId, 'font', ['objNum' => $font[0]['objNum'], 'fontNum' => $font[0]['fontNum']]);
        $fieldId = ++$this->numObj;
        $this->o_field($fieldId, 'new', ['rect' => [$x0, $y0, $x1, $y1], 'F' => 4, 'FT' => "/{$type}", 'T' => $name, 'Ff' => $ff, 'pageid' => $this->currentPage, 'da' => "{$color} /F{$this->currentFontNum} " . \sprintf('%.1F Tf ', $size)]);
        return $fieldId;
    }
    /**
     * set Field value
    ...
    public function addForm($sigFlags = 0, $needAppearances = \false)
    {
        ...
        $this->o_acroform($this->acroFormId, 'new', ['NeedAppearances' => $needAppearances ? 'true' : 'false', 'SigFlags' => $sigFlags]);
    }
    /**
    ...
    function clippingRectangleRounded($x1, $y1, $w, $h, $rTL, $rTR, $rBR, $rBL)
    {
        ...
        $this->ellipse($x1 + $rBL, $y1 + $rBL, $rBL, 0, 0, 8, 180, 270, \false, \false, \false, \true);
        // line: right edge, bottom end
        $this->addContent(\sprintf("\n%.3F %.3F l ", $x1 + $w - $rBR, $y1));
        // curve: bottom-right corner
        $this->ellipse($x1 + $w - $rBR, $y1 + $rBR, $rBR, 0, 0, 8, 270, 360, \false, \false, \false, \true);
        // line: right edge, top end
        $this->addContent(\sprintf("\n%.3F %.3F l ", $x1 + $w, $y1 + $h - $rTR));
        // curve: bottom-right corner
        $this->ellipse($x1 + $w - $rTR, $y1 + $h - $rTR, $rTR, 0, 0, 8, 0, 90, \false, \false, \false, \true);
        // line: bottom edge, right end
        $this->addContent(\sprintf("\n%.3F %.3F l ", $x1 + $rTL, $y1 + $h));
        // curve: top-right corner
        $this->ellipse($x1 + $rTL, $y1 + $h - $rTL, $rTL, 0, 0, 8, 90, 180, \false, \false, \false, \true);
    ...
    function rotate($angle, $x, $y)
    {
        ...
        $cos_a = \cos($a);
        $sin_a = \sin($a);
        $tm = [$cos_a, -$sin_a, $sin_a, $cos_a, $x - $sin_a * $y - $cos_a * $x, $y - $cos_a * $y + $sin_a * $x];
        $this->transform($tm);
    }
    /**
     * skew
    ...
    {
        $y = $this->currentPageSize["height"] - $y;
        $tan_x = \tan(\deg2rad($angle_x));
        $tan_y = \tan(\deg2rad($angle_y));
        $tm = [1, -$tan_y, -$tan_x, 1, $tan_x * $y, $tan_y * $x];
        $this->transform($tm);
    }
    /**
    ...
    function newPage($insert = 0, $id = 0, $pos = 'after')
    {
        ...
        if (isset($this->currentColor)) {
            $this->setColor($this->currentColor, \true);
        }
        if (isset($this->currentStrokeColor)) {
            $this->setStrokeColor($this->currentStrokeColor, \true);
        }
     ...
     * @return string
     */
    function filterText($text, $bom = \true, $convert_encoding = \true)
    {
        if (!$this->numFonts) {
            $this->selectFont($this->defaultFont);
        }
     ...
     */
    function utf8toUtf16BE(&$text, $bom = \true)
    {
        $out = $bom ? "\xfe\xff" : '';
        ...
        foreach ($unicode as $c) {
            if ($c === 0xfffd) {
                ...
                $out .= \chr($c >> 0x8) . \chr($c & 0xff);
            } else {
                $c -= 0x10000;
                $w1 = 0xd800 | $c >> 0x10;
                $w2 = 0xdc00 | $c & 0x3ff;
                $out .= \chr($w1 >> 0x8) . \chr($w1 & 0xff) . \chr($w2 >> 0x8) . \chr($w2 & 0xff);
            }
        }
        return $out;
    }
    ...
    function addText($x, $y, $size, $text, $angle = 0, $wordSpaceAdjust = 0, $charSpaceAdjust = 0, $smallCaps = \false)
    {
        ...
        if ($this->nCallback > 0) {
            for ($i = $this->nCallback; $i > 0; $i--) {
                // call each function
                $info = ['x' => $x, 'y' => $y, 'angle' => $angle, 'status' => 'sol', 'p' => $this->callback[$i]['p'], 'nCallback' => $this->callback[$i]['nCallback'], 'height' => $this->callback[$i]['height'], 'descender' => $this->callback[$i]['descender']];
        ...
        if ($start < $len) {
            ...
            if ($this->fonts[$this->currentFont]['isUnicode'] && $wordSpaceAdjust != 0) {
                ...
                $place_text = \str_replace("\x00 ", "\x00 )\x00 " . -\round($space_scale * $wordSpaceAdjust) . "\x00 (", $place_text);
            }
        ...
        if ($this->nCallback > 0) {
            for ($i = $this->nCallback; $i > 0; $i--) {
                ...
                $info = ['x' => $tmp[0], 'y' => $tmp[1], 'angle' => $angle, 'status' => 'eol', 'p' => $this->callback[$i]['p'], 'nCallback' => $this->callback[$i]['nCallback'], 'height' => $this->callback[$i]['height'], 'descender' => $this->callback[$i]['descender']];
                $func = $this->callback[$i]['f'];
                $this->{$func}($info);
            }
        }
    ...
    function saveState($pageEnd = 0)
    {
        if ($pageEnd) {
            ...
            $this->setColor($opt['col'], \true);
            $this->setStrokeColor($opt['str'], \true);
            $this->addContent("\n" . $opt['lin']);
            //    $this->currentLineStyle = $opt['lin'];
        } else {
            $this->nStateStack++;
            $this->stateStack[$this->nStateStack] = ['col' => $this->currentColor, 'str' => $this->currentStrokeColor, 'lin' => $this->currentLineStyle];
        }
        $this->save();
    }
    /**
    ...
    {
        $this->numObj++;
        $this->o_embedded_file_dictionary($this->numObj, 'new', ['filepath' => $filepath, 'filename' => $embeddedFilename, 'description' => $description]);
    }
    /**
    ...
    protected function addImagePngAlpha($file, $x, $y, $w, $h, $byte)
    {
        ...
        $tempfile_alpha = @\tempnam($this->tmp, "cpdf_img_");
        @\unlink($tempfile_alpha);
        $tempfile_alpha = "{$tempfile_alpha}.png";
        // create temp plain file
        $tempfile_plain = @\tempnam($this->tmp, "cpdf_img_");
        ...
        // embed mask image
        if ($tempfile_alpha) {
            $this->addImagePng($imgalpha, $tempfile_alpha, $x, $y, $w, $h, \true);
    ...
    function addPngFromFile($file, $x, $y, $w = 0, $h = 0)
    {
        ...
        if (isset($this->imageAlphaList[$file])) {
            ...
            if ($alphaFile) {
                $img = null;
                $this->addImagePng($img, $alphaFile, $x, $y, $w, $h, \true);
            }
            $img = null;
    ...
    {
        $doc = new \JFB_PDF\Vendor\Svg\Document();
        $doc->loadFile($file);
        $dimensions = $doc->getDimensions();
        $this->save();
        $this->transform([$w / $dimensions["width"], 0, 0, $h / $dimensions["height"], $x, $y]);
        $surface = new \JFB_PDF\Vendor\Svg\Surface\SurfaceCpdf($doc, $this);
        $doc->render($surface);
        $this->restore();
    }
    /**
    ...
    function addPngFromBuf(&$data, $file, $x, $y, $w = 0.0, $h = 0.0, $is_mask = \false, $mask = null)
    {
        if (isset($this->imagelist[$file])) {
            ...
            $options = ['label' => $label, 'data' => $idata, 'bitsPerComponent' => $info['bitDepth'], 'pdata' => $pdata, 'iw' => $info['width'], 'ih' => $info['height'], 'type' => 'png', 'color' => $color, 'ncolor' => $ncolor, 'masked' => $mask, 'isMask' => $is_mask];
     ...
     * @param int $channels
     */
    private function addJpegImage_common(&$data, $imgname, $imageWidth, $imageHeight, $x, $y, $w = 0, $h = 0, $channels = 3)
    {
        if ($this->image_iscached($imgname)) {
            ...
            $this->imagelist[$imgname] = ['label' => $label, 'w' => $imageWidth, 'h' => $imageHeight, 'c' => $channels];
        }
        $this->addContent(\sprintf("\nq\n%.3F 0 0 %.3F %.3F %.3F cm /%s Do\nQ ", $w, $h, $x, $y, $label));
    }
    /**
    ...
    function setFontFamily($family, $options = '')
    {
        if (!\is_array($options)) {
            if ($family === 'init') {
                ...
                $this->fontFamilies['Helvetica.afm'] = ['b' => 'Helvetica-Bold.afm', 'i' => 'Helvetica-Oblique.afm', 'bi' => 'Helvetica-BoldOblique.afm', 'ib' => 'Helvetica-BoldOblique.afm'];
                $this->fontFamilies['Courier.afm'] = ['b' => 'Courier-Bold.afm', 'i' => 'Courier-Oblique.afm', 'bi' => 'Courier-BoldOblique.afm', 'ib' => 'Courier-BoldOblique.afm'];
                $this->fontFamilies['Times-Roman.afm'] = ['b' => 'Times-Bold.afm', 'i' => 'Times-Italic.afm', 'bi' => 'Times-BoldItalic.afm', 'ib' => 'Times-BoldItalic.afm'];
            }
        } else {
