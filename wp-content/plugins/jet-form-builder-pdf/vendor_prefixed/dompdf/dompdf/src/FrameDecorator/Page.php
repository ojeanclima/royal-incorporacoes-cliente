...
use JFB_PDF\Vendor\Dompdf\Renderer;
/**
...
class Page extends AbstractFrameDecorator
{
    ...
    function check_forced_page_break(Frame $frame)
    {
        ...
        if (($frame->is_block_level() || $style->display === "table-row") && \in_array($style->page_break_before, $page_breaks, \true)) {
            // Prevent cascading splits
            $frame->split(null, \true, \true);
            $style->page_break_before = "auto";
            $this->_page_full = \true;
            $frame->_already_pushed = \true;
            return \true;
        }
        ...
        while ($prev && ($prev->is_text_node() && $prev->get_node()->nodeValue === "" || $prev->get_node()->nodeName === "bullet")) {
            $prev = $prev->get_prev_sibling();
        }
        if ($prev && ($prev->is_block_level() || $prev->get_style()->display === "table-row")) {
            if (\in_array($prev->get_style()->page_break_after, $page_breaks, \true)) {
                // Prevent cascading splits
                $frame->split(null, \true, \true);
            ...
            while ($prev_last_child && ($prev_last_child->is_text_node() && $prev_last_child->get_node()->nodeValue === "" || $prev_last_child->get_node()->nodeName === "bullet")) {
                $prev_last_child = $prev_last_child->get_prev_sibling();
            }
            if ($prev_last_child && $prev_last_child->is_block_level() && \in_array($prev_last_child->get_style()->page_break_after, $page_breaks, \true)) {
                $frame->split(null, \true, \true);
    ...
    protected function hasGap(float $childPos, Frame $frame) : bool
    {
        ...
        $contentEdge = $frame->get_position("y") + (float) $style->length_in_pt([$style->margin_top, $style->border_top_width, $style->padding_top], $cbw);
        return Helpers::lengthGreater($childPos, $contentEdge) && Helpers::lengthLessOrEqual($contentEdge, $this->bottom_page_edge);
    }
    /**
    ...
    protected function _page_break_allowed(Frame $frame)
    {
        ...
        if ($frame->is_block_level() || $display === "-dompdf-image") {
            ...
            if ($prev && ($prev->is_block_level() || $prev->get_style()->display === "-dompdf-image") && $prev->get_style()->page_break_after === "avoid") {
            ...
            if ($frame->is_inline_level()) {
                ...
                $line_number = $frame->get_containing_line() && empty($line->get_frames()) ? $line_count - 1 : $line_count;
                ...
                if ($display === "table-row") {
                    ...
                    if (!$prev) {
                        ...
                        if ($prev_group && \in_array($prev_group->get_style()->display, Table::ROW_GROUPS, \true)) {
                    ...
                    if (\in_array($display, Table::ROW_GROUPS, \true)) {
    ...
    {
        if ($this->_page_full || $frame->_already_pushed || $frame->is_text_node() && $frame->get_node()->nodeValue === "") {
            return \false;
        }
        $p = $frame;
        do {
