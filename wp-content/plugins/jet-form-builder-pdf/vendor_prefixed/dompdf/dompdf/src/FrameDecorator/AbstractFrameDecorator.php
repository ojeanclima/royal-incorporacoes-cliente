...
use JFB_PDF\Vendor\Dompdf\Frame\FrameTreeIterator;
use JFB_PDF\Vendor\Dompdf\FrameReflower\AbstractFrameReflower;
use JFB_PDF\Vendor\Dompdf\Css\Style;
use JFB_PDF\Vendor\Dompdf\Positioner\AbstractPositioner;
/**
...
abstract class AbstractFrameDecorator extends Frame
{
    ...
    {
        if ($this->content_set && $this->get_node()->nodeName === "dompdf_generated") {
    ...
    {
        if ($this->content_set && $this->get_node()->nodeName !== "body" && ($decrement = $this->get_style()->counter_increment) !== "none") {
    ...
    function prepend_child(Frame $child, $update_node = \true)
    {
    ...
    function append_child(Frame $child, $update_node = \true)
    {
        while ($child instanceof AbstractFrameDecorator) {
            $child = $child->_frame;
        }
    ...
    function insert_child_before(Frame $new_child, Frame $ref, $update_node = \true)
    {
    ...
    function insert_child_after(Frame $new_child, Frame $ref, $update_node = \true)
    {
    ...
    function remove_child(Frame $child, $update_node = \true)
    {
        while ($child instanceof AbstractFrameDecorator) {
            $child = $child->_frame;
        }
     ...
     * @return AbstractFrameDecorator
     */
    function get_parent($use_cache = \true)
    {
    ...
    public function split(?Frame $child = null, bool $page_break = \false, bool $forced = \false) : void
    {
        ...
        $this->is_split = \true;
        $split->is_split_off = \true;
        $split->_already_pushed = \true;
