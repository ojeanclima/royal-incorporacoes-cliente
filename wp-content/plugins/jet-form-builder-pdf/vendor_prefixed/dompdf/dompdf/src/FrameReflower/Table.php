...
class Table extends AbstractFrameReflower
{
    ...
    function reflow(BlockFrameDecorator $block = null)
    {
        ...
        $offset_x = (float) $left + (float) $style->length_in_pt([$style->padding_left, $style->border_left_width], $cb["w"]);
        $offset_y = (float) $style->length_in_pt([$style->margin_top, $style->border_top_width, $style->padding_top], $cb["w"]);
    ...
    public function get_min_max_width() : array
    {
        ...
        $dims = [$style->border_left_width, $style->border_right_width, $style->padding_left, $style->padding_right];
        ...
        return $this->_min_max_cache = [$min_width, $max_width, "min" => $min_width, "max" => $max_width];
    }
}
