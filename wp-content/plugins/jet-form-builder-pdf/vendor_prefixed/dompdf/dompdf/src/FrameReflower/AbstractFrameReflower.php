...
use JFB_PDF\Vendor\Dompdf\Frame\Factory;
use JFB_PDF\Vendor\Dompdf\FrameDecorator\AbstractFrameDecorator;
use JFB_PDF\Vendor\Dompdf\FrameDecorator\Block;
/**
 * Base reflower class
 *
...
abstract class AbstractFrameReflower
{
            ...
            case "absolute":
                ...
                if ($parent !== $frame->get_root()) {
                    ...
                    if ($parent_style->height === "auto") {
                        ...
                        $containing_block_height = $parent_containing_block["h"] - (float) $parent_style->length_in_pt([$parent_style->margin_top, $parent_style->margin_bottom, $parent_style->border_top_width, $parent_style->border_bottom_width], $parent_containing_block["w"]);
                    } else {
    ...
    protected function _collapse_margins() : void
    {
        ...
        if (!$frame->is_in_flow() || $frame->is_inline_level() || $frame->get_root() === $frame || $frame->get_parent() === $frame->get_root()) {
            return;
        }
    ...
    private function get_collapsed_margin_length(float $l1, float $l2) : float
    {
        if ($l1 < 0 && $l2 < 0) {
            return \min($l1, $l2);
            // min(x, y) = - max(abs(x), abs(y)), if x < 0 && y < 0
        }
        if ($l1 < 0 || $l2 < 0) {
            return $l1 + $l2;
            // x + y = x - abs(y), if y < 0
    ...
    protected function resolve_min_width(?float $cbw) : float
    {
        ...
        return $min_width !== "auto" ? $style->length_in_pt($min_width, $cbw ?? 0) : 0.0;
    }
    /**
    ...
    protected function resolve_max_width(?float $cbw) : float
    {
        ...
        return $max_width !== "none" ? $style->length_in_pt($max_width, $cbw ?? \INF) : \INF;
    }
    /**
    ...
    protected function resolve_min_height(?float $cbh) : float
    {
        ...
        return $min_height !== "auto" ? $style->length_in_pt($min_height, $cbh ?? 0) : 0.0;
    }
    /**
    ...
    protected function resolve_max_height(?float $cbh) : float
    {
        ...
        return $max_height !== "none" ? $style->length_in_pt($style->max_height, $cbh ?? \INF) : \INF;
    }
    /**
    ...
    public function get_min_max_child_width() : array
    {
        ...
        for ($iter = $this->_frame->get_children(); $iter->valid(); $iter->next()) {
            ...
            while ($iter->valid() && ($iter->current()->is_inline_level() || $iter->current()->get_style()->display === "-dompdf-image")) {
                ...
                if (\in_array($child->get_style()->white_space, ["pre", "nowrap"], \true)) {
    ...
    public function get_min_max_width() : array
    {
        ...
        $dims = [$style->padding_left, $style->padding_right, $style->border_left_width, $style->border_right_width, $style->margin_left, $style->margin_right];
    ...
    protected function _parse_string($string, $single_trim = \false)
    {
        ...
        $string = \preg_replace_callback("/\\\\([0-9a-fA-F]{0,6})/", function ($matches) {
            return \JFB_PDF\Vendor\Dompdf\Helpers::unichr(\hexdec($matches[1]));
        }, $string);
        return $string;
    }
    /**
    ...
    protected function _parse_quotes() : array
    {
        ...
        foreach ($matches as $_quote) {
            $quotes_array[] = $this->_parse_string($_quote[0], \true);
        }
    ...
    protected function _parse_content() : string
    {
        ...
        foreach ($content as $val) {
            // String
            if (\in_array(\mb_substr($val, 0, 1), ['"', "'"], \true)) {
