 ...
 */
namespace JFB_PDF\Vendor\Dompdf\FrameReflower;

use JFB_PDF\Vendor\Dompdf\FrameDecorator\AbstractFrameDecorator;
use JFB_PDF\Vendor\Dompdf\FrameDecorator\Block as BlockFrameDecorator;
use JFB_PDF\Vendor\Dompdf\FrameDecorator\TableCell as TableCellFrameDecorator;
use JFB_PDF\Vendor\Dompdf\FrameDecorator\Text as TextFrameDecorator;
use JFB_PDF\Vendor\Dompdf\Exception;
use JFB_PDF\Vendor\Dompdf\Css\Style;
use JFB_PDF\Vendor\Dompdf\Helpers;
/**
...
class Block extends AbstractFrameReflower
{
    ...
    protected function _calculate_width($width)
    {
        ...
        // Handle 'auto' values
        $dims = [$style->border_left_width, $style->border_right_width, $style->padding_left, $style->padding_right, $width !== "auto" ? $width : 0, $rm !== "auto" ? $rm : 0, $lm !== "auto" ? $lm : 0];
    ...
    protected function _calculate_restricted_height()
    {
        ...
        if ($frame->is_absolute()) {
            ...
            $h_dims = [$top !== "auto" ? $top : 0, $height !== "auto" ? $height : 0, $bottom !== "auto" ? $bottom : 0];
            $w_dims = [$style->margin_top !== "auto" ? $style->margin_top : 0, $style->padding_top, $style->border_top_width, $style->border_bottom_width, $style->padding_bottom, $style->margin_bottom !== "auto" ? $style->margin_bottom : 0];
            $sum = (float) $style->length_in_pt($h_dims, $cb["h"]) + (float) $style->length_in_pt($w_dims, $cb["w"]);
            $diff = $cb["h"] - $sum;
    ...
    function vertical_align()
    {
        ...
        foreach ($this->_frame->get_line_boxes() as $line) {
            ...
            foreach ($line->frames_to_align() as $frame) {
                $style = $frame->get_style();
                $isInlineBlock = $style->display !== "inline" && $style->display !== "-dompdf-list-bullet";
                ...
                if ($isInlineBlock) {
                    ...
                    if (\in_array($align, Style::VERTICAL_ALIGN_KEYWORDS, \true)) {
                        switch ($align) {
                            case "middle":
                            ...
                            case "super":
                                ...
                                break;
                            case "text-top":
                                // FIXME: this should be the height of the frame minus the height of the text
    ...
    function process_float(AbstractFrameDecorator $child, $cb_x, $cb_w)
    {
        ...
        if ($child_style->float !== "none") {
            ...
            $child->move($float_x - $old_x, $float_y - $old_y, \true);
        }
    }
    /**
    ...
    function reflow(BlockFrameDecorator $block = null)
    {
        ...
        $top = (float) $style->length_in_pt([$margin_top !== "auto" ? $margin_top : 0, $style->border_top_width, $style->padding_top], $cb["w"]);
        $bottom = (float) $style->length_in_pt([$margin_bottom !== "auto" ? $margin_bottom : 0, $style->border_bottom_width, $style->padding_bottom], $cb["w"]);
        $cb_x = $x + (float) $margin_left + (float) $style->length_in_pt([$style->border_left_width, $style->padding_left], $cb["w"]);
        $cb_y = $y + $top;
        $height = $style->length_in_pt($style->height, $cb["h"]);
        if ($height === "auto") {
        ...
        if ($this->_frame->is_absolute()) {
            ...
            if ($auto_margin_top) {
                $this->_frame->move(0, $margin_top, \true);
            }
        }
        $this->_text_align();
        $this->vertical_align();
