...
use JFB_PDF\Vendor\Dompdf\Frame\FrameTree;
use JFB_PDF\Vendor\Dompdf\Image\Cache;
use JFB_PDF\Vendor\Dompdf\Css\Stylesheet;
use JFB_PDF\Vendor\Dompdf\Helpers;
use JFB_PDF\Vendor\Masterminds\HTML5;
/**
...
class Dompdf
{
    ...
    public function loadHtml($str, $encoding = null)
    {
        ...
        if ($encoding === null) {
            ...
            if (($encoding = \mb_detect_encoding($str, null, \true)) === \false) {
        ...
        try {
            ...
            $quirksmode = \false;
            $html5 = new HTML5(['encoding' => $encoding, 'disable_html_ns' => \true]);
                    ...
                    if (\mb_strtolower(\stripos($tag->getAttribute("rel"), "stylesheet") !== \false) || \mb_strtolower($tag->getAttribute("type")) === "text/css") {
                        ...
                        if (\count($formedialist) > 0) {
                            ...
                            foreach ($formedialist as $type) {
                                if (\in_array(\mb_strtolower(\trim($type)), $acceptedmedia)) {
                                    $accept = \true;
                                    break;
                                }
                            }
    ...
    public function parseDefaultView($value)
    {
        ...
        $options = \preg_split("/\\s*,\\s*/", \trim($value));
    ...
    public function render()
    {
        ...
        if ($title->length) {
            $canvas->add_info("Title", \trim($title->item(0)->nodeValue));
        }
        ...
        foreach ($metas as $meta) {
            ...
            $value = \trim($meta->getAttribute("content"));
    ...
    private function writeLog(string $logOutputFile, float $startTime) : void
    {
        ...
        $memory = \memory_get_peak_usage(\true) / 1024;
        $time = (\microtime(\true) - $startTime) * 1000;
