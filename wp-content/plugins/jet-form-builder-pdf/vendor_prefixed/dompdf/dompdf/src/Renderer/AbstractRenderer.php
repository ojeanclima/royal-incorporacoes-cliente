 ...
 */
namespace JFB_PDF\Vendor\Dompdf\Renderer;

use JFB_PDF\Vendor\Dompdf\Adapter\CPDF;
use JFB_PDF\Vendor\Dompdf\Css\Color;
...
abstract class AbstractRenderer
{
    ...
    protected function _render_border(Frame $frame, array $border_box, string $corner_style = "bevel") : void
    {
        ...
        if ($bp["top"]["style"] === "solid" && $bp["top"] === $bp["right"] && $bp["right"] === $bp["bottom"] && $bp["bottom"] === $bp["left"] && !$style->has_border_radius()) {
            $props = $bp["top"];
        ...
        // Do it the long way
        $widths = [(float) $style->length_in_pt($bp["top"]["width"]), (float) $style->length_in_pt($bp["right"]["width"]), (float) $style->length_in_pt($bp["bottom"]["width"]), (float) $style->length_in_pt($bp["left"]["width"])];
        foreach ($bp as $side => $props) {
            if ($props["style"] === "none" || $props["style"] === "hidden" || $props["color"] === "transparent" || $props["width"] <= 0) {
                continue;
            }
            [$x, $y, $w, $h] = $border_box;
    ...
    protected function _background_image($url, $x, $y, $width, $height, $style)
    {
        ...
        list($img, $type, ) = Cache::resolve_url($url, $sheet->get_protocol(), $sheet->get_host(), $sheet->get_base_path(), $this->_dompdf->getOptions());
        ...
        list($img_w, $img_h) = $this->_resize_background_image($img_w, $img_h, $bg_width, $bg_height, $style->background_size, $dpi);
        ...
        $cached = $this->_canvas instanceof CPDF && $this->_canvas->get_cpdf()->image_iscached($cpdfKey) || $tmpFile !== null && \file_exists($tmpFile);
        if (!$cached) {
            ...
            case "top":
                if ($cornerClip) {
                    $points = [
                        ...
                        $x,
                        $y - 1,
                        // Extend outwards to avoid gaps
                        $x + $length,
                        $y - 1,
                        // Extend outwards to avoid gaps
                        $x + $length,
                        $y,
                        $x + $length - \max($right, $r2),
                        $y + \max($width, $r2),
                        $x + \max($left, $r1),
                        $y + \max($width, $r1),
                    ];
    ...
    protected function _resize_background_image($img_width, $img_height, $container_width, $container_height, $bg_resize, $dpi)
    {
        ...
        if (\is_array($bg_resize)) {
            ...
            if ($bg_resize === 'cover' || $bg_resize === 'contain') {
                ...
                if ($bg_resize === 'cover' && $container_ratio > $img_ratio || $bg_resize === 'contain' && $container_ratio < $img_ratio) {
