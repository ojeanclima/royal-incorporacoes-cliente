...
use JFB_PDF\Vendor\Dompdf\FrameDecorator\Table;
/**
 * Renders table cells
 *
 * @package dompdf
...
class TableCell extends Block
{
    ...
    function render(Frame $frame)
    {
        ...
        if (\trim($frame->get_node()->nodeValue) === "" && $style->empty_cells === "hide") {
            return;
        }
    ...
    protected function _render_collapsed_border(Frame $frame, Table $table) : void
    {
        ...
        foreach ($cells["columns"] as $j) {
            ...
            if ($bp["top"]["width"] > 0) {
                $widths = [(float) $bp["top"]["width"], (float) $bp["right"]["width"], (float) $bp["bottom"]["width"], (float) $bp["left"]["width"]];
